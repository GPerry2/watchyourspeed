...
// Added Form Tracking with modification to track button clicks too
// Added ihtml tracking for click events
// Modified logic for offsite files. Before they sent 2 hits for each click (one for offsite, one for file/rightclick). Now the logic only sends one hit per click
// Removed escape logic from Anchor links & modified WT.ti value to include escaped text rather than hash tag text
// Added code to extract search term on form submit and append it to the hit ** Code was escaped since this was implemented on the site with greater accuracy...
// Added code to track onsite link to internal domains (app, secure, efun, wx, map, booking) and better logic to reduce doublecounting
...
(function (_window, _document, _navigator, _location) {
    ...
    Webtrends.dcs.prototype = {
        init: function (config) {
            ...
            if (sd('formbutton', false)) this.addFormButtonTracking(); //Added this line to add formbutton tracking
            if (sd('offsite', false)) this.addOffsiteTracking();
            if (sd('onsite', false)) this.addOnsiteTracking();
            ...
            if (sd('mailto', false)) this.addMailToTracking(); //Added this line to add mailto tracking
        ...
        addSelector: function (selector, o_in) {
            ...
            var o_base = {
                ...
                'transform': undefined,
                'filter': undefined,
                'actionElems': { 'A': 1, 'INPUT': 1, 'BUTTON':1 }, // You can pass more TagNames in for additional elements. Tag Names must be uppercase.
                'finish': undefined
            };
        ...
        getURIArrFromEvent: function (e) {
            ...
            res.dcsqry = a.search ? "?" + a.search.substring(a.search.indexOf("?") + 1 , a.search.length) : ""; //Added "?" + to ensure the parameter value begins with a question mark (?)
        ...
        getTTL: function (ev, el, alt) {
            ...
            var ttl;
            if (img && img.alt) {
                ttl = "IMG: " + img.alt; //Added ["IMG: " +] to prefix IMG: to all image clicks
            } else if (text) {
                ttl = text;
        ...
        addOffsiteTracking: function () {
            ...
            this.addSelector('a', {
                filter: function (dcsObject, o) {
                    var e = o['element'] || {};
                    var evt = o['event'] || {};
                    if (e.hostname && !dcsObject.dcsIsOnsite(e.hostname, dcsObject._onsitedoms) && !dcsObject._isRightClick(evt))
                        return false;
                    else
                        return true;
                },
                transform: function (dcsObject, o) {
                    ...
                    var ttl = dcsObject.getTTL(e, el, res.dcsuri); //Added this line to set TTL variable
                    o['argsa'].push(
                        "DCS.dcssip", res.dcssip,
                        "DCS.dcsuri", res.dcsuri,
                        "DCS.dcsqry", res.dcsqry,
                        "DCS.dcsref", res.dcsref,
                        "WT.ti", "Offsite:" + res.dcssip + res.dcsuri + res.dcsqry, //AModified concatination, replaced (res.dcsqry.length ? ("?" + res.dcsqry) : "") with res.dcsqry since we are now recording ? at the beginning of on query values
                        "WT.ihtml", ttl, //Added this line to populate tag in request
                        "WT.dl", "24",
...
//Added Form Tracking Code
        addFormButtonTracking: function() {
            this.addSelector('button, input', {
                filter: function(dcsObject, o) {
                    var evt = o['event'] || {};
                    var e = o['element'] || {};
                    if (evt&&((typeof(evt.which)!="number")||(evt.which==1))){
                        var type=e.type||"";
                        if (type&&((type=="submit")||(type=="image")||(type=="button")||(type=="reset"))||
                ...
                transform: function (dcsObject, o) {
                    ...
                    dcsObject._autoEvtSetup(o);
                    var res = dcsObject.getURIArrFromEvent(el);
                    var ttl2 = dcsObject.getTTL(e, el, res.dcsuri); //Added this line to set TTL variable. Changed from ttl to ttl2 27-JAN-15
                    var id=0;
                    if (el.form){
                        res.dcsuri=el.form.getAttribute("action")||res.dcsuri; //Add to get hardcoded URI
                        // to get full url replace getAttribute("action") with action
                        ttl=el.form.id||el.form.name||el.form.className||ttl;
                        id=(el.form.method&&(el.form.method.toLowerCase()=="post"))?"27":"26";
                    }
                    else {
                        ttl=el.name||el.id||ttl;
                        id=(el.tagName.toLowerCase()=="input")?"28":"29";
                    }
// 12-FEB-14: capture search-term for toronto.ca
// Removed 17-APR-15    var srchtrm;
// Removed 17-APR-15    if(res.dcsuri.indexOf('search.toronto.ca') > -1) { srchtrm = document.getElementById("q").value; };
                    o['argsa'].push(
                        "DCS.dcssip", "",
                        "DCS.dcsuri", res.dcsuri,
                        "DCS.dcsqry", "",
                        ...
                        "WT.dl", id,
                        "WT.nv", dcsObject.dcsNavigation(e, dcsObject.navigationtag),
// Removed 17-APR-15    "WT.search-term", srchtrm,
                        "WT.btnhtml",ttl2 //Added new parameter to capture button html click text 27-JAN-15
        ...
        addAnchorTracking: function (evt) {
            this.addSelector('a', {
                ...
                transform: function (dcsObject, o) {
                    ...
                    o['argsa'].push(
                        "DCS.dcssip", res.dcssip,
                        "DCS.dcsuri", res.dcsuri + o['element'].hash, //Removed Escape logic
                        "DCS.dcsqry", res.dcsqry,
                        "DCS.dcsref", res.dcsref,
                        "WT.ti", "Anchor:" + ttl, //Removed Escape logic & replaced hash with text
                        "WT.ihtml", ttl, //Added this line to populate tag in request
                        "WT.nv", dcsObject.dcsNavigation(e, dcsObject.navigationtag),
                        "WT.dl", "21");
                },
        ...
        addDownloadTracking: function () {
            this.addSelector('a', {
                filter: function (dcsObject, o) {
                    var e = o['element'] || {};
                    var evt = o['event'] || {};
                    if (dcsObject.dcsTypeMatch(e.pathname, dcsObject._downloadtypes) && dcsObject.dcsIsOnsite(e.hostname, dcsObject._onsitedoms) &&
...
//Added Email Tracking Code
        addMailToTracking: function () {
            this.addSelector('a', {
                filter: function (dcsObject, o) {
                    var e = o['element'] || {};
                    var evt = o['event'] || {};
                    if (e.href && e.protocol && e.protocol.toLowerCase() === "mailto:")
                        return false;
                    else
                        return true;
                },
                transform: function (dcsObject, o) {
                    ...
                    o['argsa'].push(
                        ...
                        "DCS.dcsqry", res.dcsqry,
                        "DCS.dcsref", res.dcsref,
                        "WT.ti", "MailTo:" + ((el.innerHTML) ? el.innerHTML : ""),
                        "WT.ihtml", ttl, //Added this line to populate tag in request
                        "WT.dl", "23",
                        "WT.nv", dcsObject.dcsNavigation(e, dcsObject.navigationtag));
                },
...
//End of Email Tracking Code
//Start of onsite link tracking
        addOnsiteTracking: function () {
            this.addSelector('a', {
                filter: function (dcsObject, o) {
                    var e = o['element'] || {};
                    var evt = o['event'] || {};
                    if (e.hostname && (function() {
                            var allowed = ["app.toronto.ca", "booking.toronto.ca", "efun.toronto.ca", "map.toronto.ca", "secure.toronto.ca", "wx.toronto.ca"];
                            for(keys in allowed) {
                                if((e.hostname.indexOf(allowed[keys])!==-1) &&  (document.domain.indexOf(allowed[keys]) === -1)  ) { return true; }
                            }
                            return false;
                        })() && !dcsObject._isRightClick(evt))
                        return false;
                ...
                transform: function (dcsObject, o) {
                    ...
                    o['argsa'].push(
                        ...
                        "DCS.dcsqry", res.dcsqry,
                        "DCS.dcsref", res.dcsref,
                        "WT.ti", "Onsite:" + res.dcssip + res.dcsuri + res.dcsqry, //A Modified concatination, replaced (res.dcsqry.length ? ("?" + res.dcsqry) : "") with res.dcsqry since we are now recording ? at the beginning of on query values
                        "WT.ihtml", ttl, //Added this line to populate tag in request
                        "WT.dl", "204", //Created new event 204 to report on onsite clicks
                        "WT.nv", dcsObject.dcsNavigation(e, dcsObject.navigationtag)
                    );
                },
//End of Onlsite link tracking
                finish: function (dcsObject, o) {
    ...
    Webtrends.dcs.prototype['action_multitrack_multitrack'] = Webtrends.dcs.prototype.action_multitrack;
